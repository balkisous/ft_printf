...
void	ft_size_intprecision(struct s_env *p)
{
	if (p->f.intprecision < 0)
		p->f.intprecsion = 0;
	else if (p->f.size_arg < p->f.intprecsion)
		p->f.intprecision = p->f.intprecsiion - p->f.size_arg;
	else
		p->f.intprecision = 0;
}
...
void	ft_size_largeur_and_intprecision_under_0(struct s_env *p)
{
	...
	{
		p->f.largeur = -p->f.largeur;
		p->f.largeur -= p->f.intprecsion;
		p->f.intrprecision -= p->f.size_arg;
	...
	else if (p->f.largeur < 0 && p->f.intprecsision < 0)
